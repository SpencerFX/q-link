//====================================================================
// Performance report: analytics/markOutImpact.q — .util.*, .markout.*,
// .impact.* — timed against realistic synthetic data.
//
...
.perf.priv.seedPending[`.markout.pending;.markout.onTrade;500;
  {[x] `tradeID`tradeTime`tradeRate`sym!(100000+x;.perf.data.trades[`tradeTime][x];1.1;`EURUSD)}];
...
.markout.completed:0#.markout.completed;
...
.perf.priv.seedPending[`.markout.pending;.markout.onTrade;500;
  {[x] `tradeID`tradeTime`tradeRate`sym!(200000+x;.perf.data.trades[`tradeTime][x];1.1;`EURUSD)}];
...
.perf.priv.seedPending[`.impact.pending;.impact.onOrder;500;
  {[x] `orderID`orderTime`orderRate`sym`side!(100000+x;.perf.data.book[`time][x];1.1;`EURUSD;`buy)}];
...
.impact.completed:0#.impact.completed;

.perf.priv.seedPending[`.impact.pending;.impact.onOrder;500;
  {[x] `orderID`orderTime`orderRate`sym`side!(200000+x;.perf.data.book[`time][x];1.1;`EURUSD;`buy)}];
...
//====================================================================
-1 "";
-1 "Dataset sizes: rate=",(string count .perf.data.rate),
